...
module.exports = (app) => {
  ...
  router.get("/admins/all", async (req, res, next) => {
    ...
    try {
      ...
      let sql1 =
        "SELECT email, name, status, tel, DATE_FORMAT(reg_date, '%Y-%m-%d') as reg_date FROM admins";

      let args1 = [];

      if (query != null) {
        sql1 += " WHERE email LIKE ('%', ?, '%')";
        args1.push(query);
      }
  ...
  router.delete("/admims/getout", async (req, res, next) => {
    ...
    try {
      ...
      let sql1 = "UPDATE admins SET status='N' WHERE email=? AND password=?";
      let args1 = [email, password];
